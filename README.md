...
If you need use customized `server.xml`, eg. because you want to run Bamboo behind a proxy, you can easily do it by mounting it from host filesystem into the container.
Assuming that your custom `server.xml` file is present in the current working directory just add following option to the `docker run` command:

    -v $(pwd)/server.xml:/opt/atlassian/bamboo/conf/server.xml

Note that you must use absolute path otheriwse directory will be created.
